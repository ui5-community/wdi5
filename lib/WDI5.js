...
module.exports = class WDI5 {
    ...
    /**
     * retrieves the webelement id for an array of ui5 control ids
     * @param {Array} aControls UI5
     ...
     * @return {WDI5} this for method chaining
     */
    __fireEvent(eventName, oOptions) {
    ...
    _executeControlMethod(methodName, webElement = this._webElement, context = this._context, ...args) {
            ...
            (webElement, methodName, args, done) => {
                window.bridge.waitForUI5().then(() => {
                    ...
                    if (Array.isArray(result)) {
                        // espect the method call was an kind of 'getAggregation' -> retrieve the ids
                        result = window.wdi5.createControlIdMap(result);
                        done(['success', result, "aggregation"]);
                    } else {

                        if (result === undefined || result === null) {
                            done(['error', `function ${methodName} not existent in control !`, "none"]);
                        } else {
        ...
        switch (result[2]) {
            case "element":
                // identifier to return after a called mehtod the wdi5 instance to allow method chaining
                return this;
            case "result":
