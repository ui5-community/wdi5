...
module.exports = class WDI5 {
    ...
    fireEvent(eventName, oOptions) {
        return this._fireEvent(eventName, oOptions)
    }
            ...
            (ui5Control, aggregationName, done) => {
                window.bridge.waitForUI5().then(() => {
                    ...
                    try {
                        ...
                        // in case the getAggregation or map function fail
            ...
            (ui5Control, propertyName, propertyValue, done) => {
                window.bridge.waitForUI5().then(() => {
                    ...
                    try {
                        // DOM to UI5
                        let oControl = jQuery(ui5Control).control(0)
                        result = oControl.setProperty(propertyName, propertyValue)
                        done(["succes", true])
                    } catch (e) {
                        jQuery.sap.log.error(`[browser wido-ui5] setProperty failed because of: ${result}`)
                        done(["error", result.toString()])
    ...
    _hasStyleClass(className, ui5Control = this._ui5Control, context = this._context) {
        const result = context.executeAsync((ui5Control, className, done) => {
            window.bridge.waitForUI5().then(() => {
                try {
        ...
        const result = context.executeAsync(
            (ui5Control, propertyName, done) => {
                window.bridge.waitForUI5().then(() => {
                    ...
                    if (!sProperty) {
                        done(["error", "property not existent in control !"])
                    }
     ...
     * @param {WebdriverIO.BrowserObject} context
     */
    _fireEvent(eventName, oOptions, ui5Control = this._ui5Control, context = this._context) {
        const result = context.executeAsync(
            (ui5Control, eventName, oOptions, done) => {
                window.bridge.waitForUI5().then(() => {
                    // DOM to ui5
                    let oControl = jQuery(ui5Control).control(0)
                    ...
                    if (oControl && oControl.hasListeners(eventName)) {
                        ...
                        try {
                            let result = oControl.fireEvent(eventName, oOptions);
                            // return boolean
                            done(["success", true])
                        } catch (e) {
                            done(["error", e.toString()])
                        ...
                        done(["error", "control does not have the requested listener: " + eventName])
                    }
                })
            },
            ui5Control,
    ...
    _getControl(controlSelector = this._controlSelector, context = this._context) {
        const result = context.executeAsync((controlSelector, done) => {
            ...
            window.bridge
                .waitForUI5()
                .then(() => {
                    jQuery.sap.log.debug("[browser wdio-ui5] locating " + JSON.stringify(controlSelector))
                ...
                .then(domElement => {
                    jQuery.sap.log.debug("[browser wdio-ui5] control located! - Message: " + JSON.stringify(domElement))
    ...
    _writeResultLog(result, functionName) {
        if (result[0] === 'error') {
            ...
            logger.log(`SUCCESS: call of function ${functionName} retuned: ${JSON.stringify(result[1])}`)
        } else {
            logger.warn(`Unknown status: ${functionName} retuned: ${JSON.stringify(result[1])}`)
        }
    }
}
