...
module.exports = class WDI5 {
    ...
    constructor(controlSelector, context) {
        ...
        this._webElement = this._getControl();
    ...
    /**
     * @return the webdriver JSON Object
     */
    getWebControl() {
    ...
    _hasStyleClass(className, webElement = this._webElement, context = this._context) {
        const result = context.executeAsync(
            (webElement, className, done) => {
                window.bridge.waitForUI5().then(() => {
                    try {
                        const foundwebElement = window.wdi5.getUI5CtlForWebObj(webElement);
                        done(['success', foundwebElement.hasStyleClass(className)]);
                    } catch (e) {
                        done(['error', e.toString()]);
                    }
                });
    ...
    _interactWithControl(oOptions, context = this._context) {
        const result = context.executeAsync((oOptions, done) => {
            window.bridge
                .waitForUI5()
                .then(() => {
                    console.info('[browser wdio-ui5] locating controlSelector');
                    oOptions.selector = window.wdi5.createMatcher(oOptions.selector);
                    return window.bridge.interactWithControl(oOptions);
                })
                .then((result) => {
                    console.info('[browser wdio-ui5] interaction complete! - Message: ' + result);
                    done(['success', result]);
                })
                .catch((error) => {
     ...
     * @param {WebdriverIO.BrowserObject} context
     */
    _fireEvent(eventName, webElement = this._webElement, context = this._context) {
        const result = context.executeAsync(
            (webElement, eventName, done) => {
                window.bridge.waitForUI5().then(() => {
                    console.info('[browser wdio-ui5] working ' + eventName + ' for ' + webElement);
                    // DOM to ui5
                    let oControl = window.wdi5.getUI5CtlForWebObj(webElement);
                    if (oControl && oControl.hasListeners(eventName)) {
                        console.info('[browser wdio-ui5] firing ' + eventName + ' on ' + webElement);
                        // element existent and has the target event
                        try {
                            oControl.fireEvent(eventName);
                            // convert to boolean
                            done(['success', true]);
                        } catch (e) {
                            done(['error', e.toString()])
                        ...
                        console.error('[browser wdio-ui5] couldn\'t find ' + webElement);
                        done(['error', false]);
                    }
                });
            },
