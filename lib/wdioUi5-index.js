        ...
        if (!window.wdi5 && !window.bridge) {
            ...
            sap.ui.require(
                ...
                (BindingPath, I18NText, Properties, Ancestor, LabelFor) => {
                    ...
                    /**
                    * Extracts strings of control function of control parameter
                    * @returns {Array} Type Strings
                    * @param {sap.<lib>.<Control>} control
                    */
                    window.wdi5.createControlMethod = (control) => {

                        // create keys of the direct parent prototype
                        let sAllFunctionNames = Object.keys(control.__proto__);

                        // filter relevant function names
                        let sReplFunctionNames = sAllFunctionNames.filter((f) => {

                            if (f.indexOf("_") === -1) {
                                // no private methods
                                if ((f.indexOf("get") !== -1) || (f.indexOf("set") !== -1)) {
                                    // getters and setters
                                    if (f.indexOf("Metadata") !== -1) {
                                ...
                                if ((f.indexOf("on") !== -1) && (f.indexOf("constructor") === -1) && (f.indexOf("Render") === -1)) {
                                    // no constructor, no e.g. onAfterRendering
                                    // TODO:
                                    return false
                                }
                            }
...
function setup(context) {
    ...
    function createWdioUI5KeyFromSelector(selector) {
        ...
        const wdi5_ui5_key = stripNonValidCharactersForKey(`${orEmpty(selector.id)}_${orEmpty(selector.viewName)}_${orEmpty(selector.controlType)}_${orEmpty(JSON.stringify(selector.bindingPath))}_${orEmpty(JSON.stringify(selector.I18NText))}_${orEmpty(selector.labelFor)}_${orEmpty(JSON.stringify(selector.properties))}`);
        return wdi5_ui5_key;
    };
