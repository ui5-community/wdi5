 ...
 * @param {WebdriverIO.BrowserObject} context
 */
function _getSelectorForElement(domElement, context) {
    const result = context.executeAsync((domElement, done) => {

        return window.bridge
            .waitForUI5()
            .then(() => {
                console.log("[browser wdio-ui5] locating domElement");
                return window.bridge.findControlSelectorByDOMElement(domElement)
            })
...
function _attachCommands(context) {
    ...
    context.addCommand("getSelectorForElement", domElement => {
        let result = _getSelectorForElement(domElement, context)
