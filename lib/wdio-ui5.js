 ...
 * @param {*} context
 */
function _hasStyleClass(ui5Control, name, context) {
    const result = context.executeAsync(
        (ui5Control, name, done) => {
            return window.bridge.waitForUI5().then(() => {
                let oControl = jQuery(ui5Control).control(0)
                const bResult = oControl.hasStyleClass(name) // ui5 methode auf sap.ui.core.Control
                done(bResult)
            })
        },
        ui5Control,
 ...
 * @param {*} context
 */
function fireEvent(ui5Control, name, context) {
    const result = context.executeAsync(
        (ui5Control, name, done) => {
            return window.bridge.waitForUI5().then(() => {
                // DOM to ui5
...
function _attachCommands(context) {
    ...
    context.addCommand("_hasStyleClass", async (uiControl, name) => {
        return await _hasStyleClass(uiControl, name, context)
    })

    context.addCommand("_fireEvent", async (ui5control, name) => {
        const result = await fireEvent(ui5control, name, context);
        return result;
    })

    context.addCommand("fireEvent", async (name) => {
        // TODO:
    }, true)
    ...
    context.addCommand("isFocusedDeep", () => {
    ...
    context.addCommand("setProperty", (property, value) => {
    ...
    context.addCommand("setAttribute", (attribute, value) => {
    ...
    context.addCommand("removeAttribute", (attribute) => {
    ...
    context.addCommand("hasClass", (className) => {
    ...
    context.addCommand("asControl", (control) => {
        ...
        // TODO: create object of class webui5
        return {
            ...
            hasClass(name) {
                ...
                return context._hasStyleClass(ui5control, name)
            },
            getProperty(name) {
                const ui5control = this._getControl(control)
            ...
            fireEvent(name) {
                const ui5control = this._getControl(control)
                return context._fireEvent(ui5control, name)
            },
            enterText(text) {
                // TODO:
            },
