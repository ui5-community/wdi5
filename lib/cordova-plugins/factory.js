        ...
        const result = this._context.executeAsync(
            (sSetupFunction, pluginName, done) => {
                try {
                    ...
                    // call success
                    done(['success', result.toString()]);
                } catch (e) {
                    window.wdi5.Log.error(`[browser wdio-ui5] ERR executing the plugin function: ${pluginName}`);
                    done(['error', `ERR executing the plugin function: ${pluginName}`]);
                }
            },
            sSetupFunction,
            pluginName
