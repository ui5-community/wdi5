...
async function clientSide_injectUI5(waitForUI5Timeout: number, browserInstance: WebdriverIO.Browser) {
    ...
    const useClosestTo = compare(ui5Version, "1.108.0", ">")
    const useOldMatcherAPI = compare("1.72.0", ui5Version, ">")
    const needsDoubleLeadingSlash = compare("1.81.0", ui5Version, ">")
    return await browserInstance.execute(
        async function wdi5_injectUI5(waitForUI5Timeout, useClosestTo, useOldMatcherAPI, needsDoubleLeadingSlash) {
            if (window.bridge) {
            ...
            if (!window.bridge) {
                ...
                window.wdi5.createMatcher = (oSelector) => {
                    ...
                    if (oSelector.bindingPath) {
                        ...
                        if (hasNamedModel && isRootProperty && needsDoubleLeadingSlash) {
