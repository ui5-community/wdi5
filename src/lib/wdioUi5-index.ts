...
async function checkForUI5Page() {
    browser.waitUntil(
        ...
        () => {
            const readyState = browser.executeAsync((done) => {
                setTimeout(() => {
...
function setup(context) {
    ...
    Logger.setLoglevel(browser.config?.wdi5?.logLevel || "error")
     ...
     * @param {WebdriverIO.BrowserObject} browser
     */
    browser.addCommand("getSelectorForElement", (oOptions) => {
        const result = browser.executeAsync((oOptions, done) => {
            window.bridge
     ...
     */
    browser.addCommand("getUI5Version", () => {
     ...
     */
    browser.addCommand("screenshot", (fileAppendix) => {
        _waitForUI5()
     ...
     */
    browser.addCommand("writescreenshot", (fileAppendix) => {
        _writeScreenshot(fileAppendix)
    })
     ...
     */
    browser.addCommand("setUrl", (url) => {
        browser.config.wdi5["url"] = url
     ...
     */
    browser.addCommand("goTo", (oOptions) => {
        ...
        if (sHash && sHash.length > 0) {
            const url = browser.config.wdi5["url"]
            ...
            if (url && url.length > 0 && url !== "#") {
                ...
                if (alreadyNavByHash && !navToRoot) {
                    browser.url(`${currentUrl.split("#")[0]}${sHash}`)
                } else {
                    browser.url(`${url}${sHash}`)
                }
     ...
     */
    browser.addCommand("asControl", (wdi5Selector) => {
        ...
        if (!browser._controls[internalKey] || wdi5Selector["forceSelect"]) {
            ...
            // create WDI5 control
            const wdi5Control = new WDI5(wdi5Selector, browser, wdi5Selector["forceSelect"])
...
function _writeScreenshot(fileAppendix) {
    ...
    if (browser.config.wdi5["screenshotsDisabled"]) {
    ...
    let _path = browser.config.wdi5["screenshotPath"]
    ...
    const platform = browser.config.wdi5["platform"]
