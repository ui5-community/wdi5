...
exports.config = {
    ...
    capabilities: [ //appium config
        {
            automationName: 'XCUITest',
            platformName: 'iOS',
            platformVersion: '13.5', // The version of the Android or iOS system
            deviceName: 'iPhone SE (2nd generation)', // For iOS, this must exactly match the device name as seen in Xcode.
            // Where to find the .apk or .ipa file to install on the device. The exact location of the file may change depending on your Cordova version.
            app: path.join('test', 'ui5-app', 'app', 'platforms', 'ios', 'build', 'emulator', 'UI5.app'),
            // By default, Appium runs tests in the native context. By setting autoWebview to true, it runs our tests in the Cordova context.
            autoWebview: true,
            isHeadless: false, // display sim window - seems to default to true w/ macOS 10.15.4, iOS sim 11.4.1 (921.9), appium 1.17.1
            permissions: "{\"sample.app\": {\"camera\": \"yes\",\"notifications\": \"yes\"}}"
        }
    ],
    wdi5: { // wdi5 specific config
        screenshotPath: path.join('test', 'report', 'screenshots'),
        deviceType: 'native', // native | web
        platform: 'ios',
        // -> android config currently set to verbose !!!
    ...
    before: function (capabilities, specs) {
        ...
        // to init first call need to be with the webcontext string
